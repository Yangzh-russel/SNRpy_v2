...
#
# Author:      Angel Martin and Raquel Lujan
#
# Created:     16/January/2017
# Final Modification:
#              08/April/2020
# Copyright:   (c) Angel Martin and Raquel Lujan 2017
# -------------------------------------------------------------------------------

import os
# from astropy.stats import LombScargle # to astropy V2
from astropy.timeseries import LombScargle  # from astropy V3
import numpy
import math
from math import pi
...
figPath = 'data/output2/'
# INPUT FILE
archivo = 'data/output1/SNR-GPS.txt'  # Input file

##PARAMETERS
# observation data interval (from the GPS observation)
int_tiempo = 5
...
azi_max = 360
# antenna height measured in field
h0 = 1.80
# GPS L1 or L2 wavelength of the signals. L1=0.1904, L2=0.2443
land = 0.1904
...
for q in range(len(sat_lis)):
    ...
    while True:
        ...
        # 对数据进行预处理，去除 np.float64 前缀并去除前后空格
        elev = datos[4].strip().replace('np.float64', '')
        azi = datos[5].strip().replace('np.float64', '')
        if int(sat) == int(datos[2]) and float(elev) > el_min and float(elev) < el_max and float(
                azi) > azi_min and float(azi) < azi_max:

            # Generation of the output files
            salida = open(figPath + str(sat.strip()) + '/' + 'final_values.txt', 'w')
            # Header line
            salida.write(
                'epoch good_track epoch_time month day id_sat Azi_ini aZi_end ele_ini ele_end A desvA Phi desvPhi ref_height SatelliteDirection\n')
    ...
    for i in range(0, lon):  # for every satellite track
        ...
        if len(el_lis) > (30 * 60 / int_tiempo) and (max(el_lis) - min(el_lis)) > inc_eleva:
            ...
            for i in frec_sen:
                ...
                if i * land / 2 < 2.5:  ##maximum H allowed is of 2.5 meters
                    frec2_sen.append(i * land / 2)
                    line_sec = (power_sen[aux])
                    line_frec2_sen.append(line_sec)
                    aux = aux + 1

            rh = frec2_sen[line_frec2_sen.index(max(line_frec2_sen))]  # Reflector height
            ...
            for i in range(len(x_range)):
                ...
                for j in range(len(el_lis) - 1):
                    if (x_range[i] > el_lis[j] and x_range[i] < el_lis[j + 1]) or (
                            x_range[i] < el_lis[j] and x_range[i] > el_lis[j + 1]):
            ...
            if period_cond > 6 and abs(wave_mean) <= 1.3 and wave_std < 25.0 and len(y_onda_red) > 85 and len(
                    y2_onda) < 110:
                buena = "yes"
            else:
                buena = "no"
            ...
            for s in range(len(e_lis1)):
                if s == 0:
                    ...
                    salida.write(str(e_lis1[s]) + ' ' + buena + ' ' + str(round(period_cond, 2)) + ' ' + str(
                        h_lis[s].strip()) + ' ' + str(mes_lis[s].strip()) + ' ' + str(dia_lis[s].strip()) + ' ' + str(
                        sat.strip()) + ' ' + str(ini_azi) + ' ' + str(fin_azi) + ' ' + str(ini_ele) + ' ' + str(
                        fin_ele) + ' ' + str(abs(a0)) + ' ' + str(desv_A) + ' ' + str(phase) + ' ' + str(
                        desv_p) + ' ' + str(round(rh, 3)) + ' ' + str(direction) + '\n')
                    if buena == "yes":
                        salida2.write(str(e_lis1[s]) + ' ' + buena + ' ' + str(h_lis[s].strip()) + ' ' + str(
                            mes_lis[s].strip()) + ' ' + str(dia_lis[s].strip()) + ' ' + str(sat.strip()) + ' ' + str(
                            ini_azi) + ' ' + str(fin_azi) + ' ' + str(ini_ele) + ' ' + str(fin_ele) + ' ' + str(
                            abs(a0)) + ' ' + str(desv_A) + ' ' + str(phase) + ' ' + str(desv_p) + ' ' + str(
                            round(rh, 3)) + ' ' + str(direction) + '\n')
                    epoch = str(e_lis1[s])
            ...
            font_size = 8
            nombre_dib = epoch + '-' + str(fechi.strip()) + '-' + str(sat.strip()) + '-' + str(primer_az) + '-' + str(
                final_az)
            ...
            plt.plot(x_range, (a0 * numpy.cos(((4 * pi * h0) / land) * numpy.sin(x_range * pi / 180) + p0)), 'grey',
                     label='Adjusted Wave')
            plt.xlabel('Elevation (deg.)', fontsize=font_size, fontweight='bold')
            plt.ylabel('SNR (Volts/Volts)', fontsize=font_size, fontweight='bold')
            plt.xlim([5, 30])
